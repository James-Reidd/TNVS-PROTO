...
if ($role === 'rider') {
    ...
    $check_query = "SELECT * FROM customer_tbl WHERE contact_no='$contact' OR email='$email'";
    ...
    if ($result->num_rows > 0) {
        ...
        // PHP Variables    -> ('$name', '$contact', '$email', '$password')
        // ----------------------------------------------------------------
        $sql = "INSERT INTO customer_tbl (name, contact_no, email, password) 
...
// ====================================================================
    ...
    // 2. VALIDATE INPUTS: Check for missing data or letters in the phone number
    if (empty($username) || empty($contact) || empty($email) || empty($password) || empty($plate_no) || empty($brand) || empty($model) || empty($color)) {
        echo "Please fill all fields!";
        exit();
    }
    if (!preg_match('/^[0-9]+$/', $contact)) {
    ...
    $check_query = "SELECT * FROM driver_tbl WHERE contact_no='$contact' OR email='$email'";
    $result = $conn->query($check_query);
    ...
    if ($result->num_rows > 0) {
        ...
        // ----------------------------------------------------------------
        $sql = "INSERT INTO driver_tbl (username, contact_no, email, password, plate_no, brand, model, color) 
                VALUES ('$username', '$contact', '$email', '$password', '$plate_no', '$brand', '$model', '$color')";
